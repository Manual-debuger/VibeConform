...
- [ ] `internal/resource/resource.go`: add `Mode os.FileMode` to `Resource`
...
- [ ] `internal/resource/resource_test.go` (new — the package has no test
      file yet): zero-value `Mode` resolves to `0o644` for `Generated`; an
      explicit mode is returned unchanged.
- [ ] `internal/cli/sync.go`: `writeResource` passes the resource's resolved
      mode to `atomicfile.Write` instead of the hardcoded `0o600`.
- [ ] `internal/cli/sync_test.go`: on non-Windows, a synced script resource
...
- [ ] `internal/module/agents/templates/`: byte-for-byte copies of
...
- [ ] `internal/module/agents/agents.go`: `New()`, `Name()` →
...
- [ ] `internal/module/agents/agents_test.go`: `Name()`; five resources with
...
- [ ] `internal/module/agents/templates_test.go`: each embedded template
      equals the live file it was seeded from.
- [ ] `internal/module/agents/templates_lf_test.go` (or a case in the above):
      neither hook script contains a CR byte. CRLF would break them under
      `bash`, and nothing else in the pipeline would notice.
- [ ] `internal/standard/standard.go`: register `agents.New()` after
      `repotooling.New()`.
- [ ] `internal/standard/standard_test.go`: ordered module-name assertion
      extended to four modules.
- [ ] `internal/cli/*_test.go`: update assertions that depend on the full
      report or resource count (now eleven resources).
- [ ] `docs/usage.md`: `production/v1` now manages agent configuration;
...
- [ ] `README.md`: status banner — four modules.
- [ ] `task verify` clean.
