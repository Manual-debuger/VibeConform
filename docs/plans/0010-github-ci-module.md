...
- [ ] `internal/module/ci/github/github.go`: unexported struct satisfying
...
- [ ] `internal/module/ci/github/github_test.go`: `Name()`; `Resolve`
...
- [ ] `internal/module/ci/github/templates_test.go`: each embedded template
...
- [ ] `internal/standard/standard.go`: register `github.New()` after
      `gotooling.New()` in `production`/`v1`.
- [ ] `internal/standard/standard_test.go`: assert `production`/`v1`'s
      module names in order (`go-tooling`, `github-ci`), not just a count.
- [ ] `internal/cli/sync_test.go`: add a nested-path case — sync into an
...
- [ ] `internal/cli/diff_test.go` / `audit_test.go`: existing single-resource
      ...
      count.
- [ ] `docs/usage.md`: example outputs now list four resources; note that
      `production/v1` manages `.github/` and that `release.yml` /
      `.goreleaser.yaml` deliberately stay project-owned.
- [ ] `README.md`: status banner — `production`/`v1` composes two modules.
- [ ] `task verify` clean.
